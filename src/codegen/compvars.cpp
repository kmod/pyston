...
public:
    ...
    static CompilerType* fromRT(BoxedFunction* rtfunc, bool stripfirst) {
        ...
        for (int i = 0; i < clf->versions.size(); i++) {
            ...
            // XXX
            if (type_sig->rtn_type == UNBOXED_INT)
                type_sig->rtn_type = INT;
...
public:
    ...
    void assertMatches(Unboxed* val) override final {
...
CompilerVariable* makeInt(llvm::Value* n) {
    ...
    return new IntType::VAR(&_INT, new IntType::Unboxed{ n, NULL }, true);
}
...
CompilerVariable* unboxInt(IREmitter& emitter, ConcreteCompilerVariable* v) {
    assert(v->getType() == BOXED_INT);
    llvm::Value* unboxed = emitter.getBuilder()->CreateCall(g.funcs.unboxInt, v->getValue());
    return new IntType::VAR(&_INT, new IntType::Unboxed{ unboxed, v }, true);
}

CompilerVariable* unboxInt(IREmitter& emitter, llvm::Value* v) {
    assert(v->getType() == g.llvm_value_type_ptr);
    return unboxInt(emitter, new ConcreteCompilerVariable(BOXED_INT, v, false));
}
...
public:
    PhonyUnboxedType(llvm::Type* t) : t(t) {}
    ...
    */
    ...
    ConcreteCompilerType* getBoxType() override {
        if (t == g.i64)
            return BOXED_INT;
        RELEASE_ASSERT(0, "%s", debugName().c_str());
    }
    */
...
ConcreteCompilerType* UNBOXED_INT = new PhonyUnboxedType(llvm::Type::getInt64Ty(llvm::getGlobalContext()));
